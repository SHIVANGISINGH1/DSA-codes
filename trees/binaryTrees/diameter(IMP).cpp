...
diaPair diameter2(Node* node) {
    ...
    // the ht of the new pair will be the max ht from left and right plus one 
    // basically we are calculating the height here only to reduce the complexity
    np.ht = max(pair1.ht,pair2.ht) + 1;
    
    // now diameter will be left ht + right ht + 2
    int fac = pair1.ht + pair2.ht + 2;
    
    // the max of left dia and right dia and the calculated dia will be the final diameter
